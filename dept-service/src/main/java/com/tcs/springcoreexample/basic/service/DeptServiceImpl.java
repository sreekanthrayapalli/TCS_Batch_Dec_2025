...
public class DeptServiceImpl implements DeptService{
	...
	public DeptDto getDept(int deptno) {
		/*
		 * String url = "http://localhost:8081/employee?deptno="+deptno;
		 * ResponseEntity<List<EmployeeDTO>> list=restTemplate.exchange( url,
		 * HttpMethod.GET, null, new ParameterizedTypeReference<List<EmployeeDTO>>() {}
		 * );
		 */ 
		ResponseEntity<List<EmployeeDTO>> list = 
				client.getEmployeeBasedDeptno(deptno);
